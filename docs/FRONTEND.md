...
Kompletní HTML → Prohlížeč
    │
    │  Statické JS soubory:
    │    uniforms/static/js/main.js          → sdílené helpery (apiFetch,
    │                                          escapeHtml, badgeAttrs, showToast)
    │    uniforms/static/js/uniforms.js      → UniForms renderer
    │    uniforms/static/js/record_detail.js → logika stránky detailu záznamu
    │    uniforms/static/js/records_list.js  → logika seznamu záznamů
    │      (šablona předává jen konstanty: COLLECTION_ID, RECORD_ID, …)
    │
    ▼
JavaScript init():
...
Všechny webové stránky jsou obsluhované v `uniforms/web/routes.py`. Přihlášení se ověřuje JWT cookie `uniforms_token`.
...
Dodává je context processor v `uniforms/web/routes.py` — čte konfiguraci při
každém requestu, takže respektuje i `configure()` zavolané hostitelskou aplikací
(knihovní režim):
