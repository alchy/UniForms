...
from uniforms.core.security import hash_password, require_admin
from uniforms.models.user import CollectionRoleId, User, UserCreate, UserResponse, UserUpdate
...
async def create_user(
    ...
    await db.execute(
        ...
        (request.username, await hash_password(request.password), request.role),
    )
    await db.commit()
...
async def update_user(
    ...
    # Ochrana proti zamknuti se ven z aplikace: admin si nesmi sam sobe
    # odebrat admin roli ani deaktivovat vlastni ucet
    if username == current_user.username:
        if request.role is not None and request.role != "system_admin":
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Nelze odebrat admin roli vlastnímu účtu",
            )
        if request.is_active is False:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Nelze deaktivovat vlastní účet",
            )
    ...
    if request.password is not None:
        await db.execute(
            ...
            (await hash_password(request.password), username),
        )
    await db.commit()
    row = await _get_user_row(db, username)
    return UserResponse(**dict(row))
...
class CollectionRolesUpdate(BaseModel):
    roles: dict[str, CollectionRoleId | None]  # {collection_id: role | null → odebrat}
