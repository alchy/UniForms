...
from uniforms.config import DEFAULT_JWT_SECRET, uniforms, settings as app_settings
from uniforms.core.database import get_db
from uniforms.core.security import COOKIE_NAME, WebAdminRequired, WebLoginRequired, decode_token
from uniforms.models.user import TokenPayload
from uniforms.services import collection_service
from uniforms.services.settings_service import get_all_settings, get_collections_dir
from uniforms.services.template_service import TemplateService, get_template_service
...
# Cesta k šablonám je relativní k balíčku, aby fungovala i při použití
# UniForms jako knihovny (nezávisle na pracovním adresáři hostitele).
# ---------------------------------------------------------------------------
...
# Globalni kontext z uniforms.yaml pro vsechny Jinja2 sablony;
# context processor cte konfiguraci pri kazdem requestu, takze respektuje
# i configure() zavolane hostitelskou aplikaci
...
templates = Jinja2Templates(directory=str(_TEMPLATES_DIR), context_processors=[_base_context])
...
async def _sidebar_ctx(
    ...
    collections_dir = await get_collections_dir(db)
...
async def settings_page(
    ...
    init_settings = {
        ...
        "jwt_secret_safe": "*** (default – change in .env!)" if app_settings.jwt_secret_key == DEFAULT_JWT_SECRET else "*** (custom key set)",
    }
