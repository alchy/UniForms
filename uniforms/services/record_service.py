...
from uniforms.config import settings, uniforms
from uniforms.models.collection import CollectionConfig
from uniforms.models.record import UniRecord, UpdateRecordRequest
...
def generate_record_id(collection: Optional[CollectionConfig] = None) -> str:
    """
    Generate a record ID from the configured format string.
    ...
    """
    id_cfg = collection.id_format if collection else uniforms.id
...
def _resolve_workflow_states(collection: Optional[CollectionConfig] = None) -> list[dict]:
    ...
    return [s.model_dump() for s in uniforms.workflow.states]
...
async def create_record(
    ...
    # Nahodna cast ID muze kolidovat – zkousime, dokud nenajdeme volne ID,
    # jinak by novy zaznam tise prepsal existujici soubor
    for _ in range(20):
        record_id = generate_record_id(collection)
        if not await storage.record_exists(record_id):
            break
    else:
        raise RuntimeError("Could not generate a unique record ID – ID format has too few random bits")
    now = datetime.now(timezone.utc)
